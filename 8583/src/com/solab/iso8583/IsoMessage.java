    ...
    public String debugString() {
        ...
        for (int i = 2; i < 129; i++) {
            ...
            if (v != null) {
                ...
                String lenStr = v.getValue().toString();
                if (v.getType() == IsoType.LLBIN || v.getType() == IsoType.LLVAR) {
                    sb.append(String.format("%02d", getLen(lenStr)));
                } else if (v.getType() == IsoType.LLLBIN || v.getType() == IsoType.LLLVAR) {
                    sb.append(String.format("%03d", getLen(lenStr)));
                } else if (v.getType() == IsoType.LLLLBIN || v.getType() == IsoType.LLLLVAR) {
                    sb.append(String.format("%04d", getLen(lenStr)));
                }
                //手工加的代码，判断自定义域为变长且实际长度为奇数时，末尾自动补0
                if(v.getType()==IsoType.LLVAR||v.getType() == IsoType.LLLVAR||v.getType() == IsoType.LLLLVAR){
                	if(getLen(lenStr)%2!=0){
                		sb.append(desc).append("0");
                		continue;
                	}
                }
                sb.append(desc);
